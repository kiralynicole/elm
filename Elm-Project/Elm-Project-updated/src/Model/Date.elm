...
monthsBetween dA dB =
   --  Nothing
    ...
    in    
        case (mA, mB) of 
            (Just a, Just b) -> Just (abs (( monthToInt a - monthToInt b) + 12 * (yA - yB)))
            (Nothing, Nothing) -> Just (abs (12 * (yA - yB)))
            (_, _) -> Nothing
...
compare (Date d1) (Date d2) =
   --  EQ
     chainCompare (compareTypesMonth d1.month d2.month) (Basics.compare d1.year d2.year)

   -- Debug.todo "Implement Model.Date.compare"

compareTypesMonth : Maybe Month->Maybe Month->Order
compareTypesMonth m1 m2 = 
     case(m1, m2) of
        (Just x, Just y) -> compareMonth x y
        (Just _, Nothing) -> LT 
        (Nothing, Just _) -> GT
        (Nothing, Nothing) -> EQ   
...
view (Date d) =
    let
        y = String.fromInt(year (Date d))
        m = month(Date d) |> Maybe.map monthToString |> Maybe.withDefault "not month"
    in    
     case month (Date d) of
        Just _ -> div[] [text (m ++ "-" ++ y)]
        Nothing -> div[] [text y]
