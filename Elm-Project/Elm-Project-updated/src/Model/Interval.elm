    ...
    let
        cmp = 
            case(intA.end, intB.end) of
                ...
                (Nothing, Just b) -> GT
    in
        chainCompare (cmp) (Date.compare intA.start intB.start)            
...
view interval =
    let
        ...
        lengthString (y,m) = fromInt y ++ "years" ++ fromInt m ++ "months"
        intEnd = 
            case intv.end of
                Just a -> Date.view a
                Nothing -> text "Present"
        intervalString = length interval |> Maybe.map lengthString |> Maybe.withDefault ""    
    in

        div [class "interval"] [
            p[class "interval-start"][Date.view intv.start]
            ,p[class "interval-end"][intEnd]
            ,p[class "interval-length"][text intervalString]
     ]
